...
local quantity=100
local type="lsd"
local subMaterial="sdrugs/blotters/bycicle_day"
local Ded={".5","0",".5","0"}
local Ded2={".5",".5","0","0"}
...
function ENT:Initialize()
		self:SetModel("models/sdrugs/blotter/25sheet.mdl")
...
function ENT:Use( activator, caller )	
 if IsValid(caller) and caller:IsPlayer()  then
  ...
  local Count=0
  for k, v in pairs( ents.FindByClass( "sdrugs_blotter_5sheet" ) ) do
   if v:CPPIGetOwner()==activator then
    Count=Count+1
   end
  end
  if Count>=GetConVar("sdrugs_limitspawn_5sheet"):GetInt() then
  ...
  self:Remove()
  for i=1,5 do
   local sheet5 = ents.Create("sdrugs_blotter_5sheet")
   sheet5:SetPos( Pos-sheet25:GetForward()*(4*i)+(sheet25:GetForward()*10))
   sheet5:CPPISetOwner(activator)
   sheet5:SetAngles( angles25)
   sheet5:Spawn()
   sheet5:Activate()
   sheet5:SetNWString("SDrugs_blotter_data","sdrugs_blotter_".."-"..subMaterial.."-"..tostring(i))
   sheet5:SetNWInt("matpos",matpos)
   sheet5:SetNWString("SDrugs_type",thetype)
   sheet5:SetNWInt("SDrugs_quantity",thequantity)
   duplicator.StoreEntityModifier( sheet5, "sdrugs_blotter_data", {data="sdrugs_blotter_".."-"..subMaterial.."-"..tostring(i), matpos=matpos,
