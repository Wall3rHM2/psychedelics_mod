...
local water_tip_text="[press e to water]"
local function water_bar_draw(ent)
	local level=ent:GetNWInt("psychedelics_box_level",0)
	local water = ent:GetNWInt("psychedelics_water_level",100)
	local mins, maxs = ent:GetModelBounds()
	local pos = ent:LocalToWorld(Vector(maxs.x,0,maxs.z/2))	
	local ang = ent:LocalToWorldAngles(Angle(0,90,90))	
	local bar_width=Lerp(water/100,0,300)
	cam.Start3D2D( pos, ang, 0.1)
	draw.NoTexture() --fixes material bugs
	...
	surface.DrawRect(-150,-10,bar_width,20)	
	if (level>=3&&level<=5) then
		surface.SetFont( "DermaLarge" )
		local w,h=surface.GetTextSize(water_tip_text)
		surface.SetTextColor( 230, 230, 230 )
		surface.SetTextPos( -w/2, -h/0.7 ) --center the text
		surface.DrawText( water_tip_text )
	end
	cam.End3D2D()
end
function ENT:Draw()
