...
local quantity=100
local type="lsd"
local material="psychedelics_mod/blotters/sroad"
local defaultdata="psychedelics_mod_blotter_-"..material.."-1"
local tmaterial=string.Split(material,"/")
local Ded={".5","0",".5","0"}
local Ded2={".5",".5","0","0"}
function ENT:Initialize()
	self:SetModel("models/psychedelics_mod/lsd/blotter/sheet.mdl")
	...
	timer.Simple(0.1,function()
		...
		local data=self:GetNWString("psychedelics_mod_blotter_data",defaultdata)
		local dataTab=string.Split(    data,"-"    )
		subMaterial=dataTab[2]
		local tmaterial=string.Split(subMaterial,"/")
		local sheet25=self

		duplicator.StoreEntityModifier( sheet25, "psychedelics_mod_blotter_data", {data="psychedelics_mod_blotter_".."-"..subMaterial.."-"..tostring(i), matpos=i, --saves the data
		...
		net.WriteString("0")
		net.WriteString("0")
		net.WriteString("psychedelics_mod_sheet_4".."-"..tmaterial[#tmaterial])
...
function ENT:Use( activator, caller )	
	if IsValid(activator) and activator:IsPlayer()  then
		...
		timer.Simple(0.2,function() activator:SetNWBool("psychedelics_mod_usedblotter",false) end) --delay for the player can use the blotters again
		local Count=0
		for k, v in pairs( ents.FindByClass( "psychedelics_mod_blotter_25sheet" ) ) do
			if v:CPPIGetOwner()==activator then
				Count=Count+1
			end
		end
		if Count>=GetConVar("psychedelics_mod_limitspawn_25sheet"):GetInt() then
		...
		self:Remove()
		for i=1,4 do
			local sheet25 = ents.Create("psychedelics_mod_blotter_25sheet")
			sheet25:SetPos( Pos+Vector(0,0,2*i))
			sheet25:CPPISetOwner(activator)
			sheet25:SetAngles( Angle( 0, 0, 0 ) )
			sheet25:Spawn()
			sheet25:Activate()
			sheet25:SetNWString("psychedelics_mod_blotter_data","psychedelics_mod_blotter_".."-"..subMaterial.."-"..tostring(i))
			sheet25:SetNWInt("matpos",i)
			sheet25:SetNWString("psychedelics_mod_type",thetype)
			sheet25:SetNWInt("psychedelics_mod_quantity",thequantity)
			duplicator.StoreEntityModifier( sheet25, "psychedelics_mod_blotter_data", {data="psychedelics_mod_blotter_".."-"..subMaterial.."-"..tostring(i), matpos=i, --saves the data
