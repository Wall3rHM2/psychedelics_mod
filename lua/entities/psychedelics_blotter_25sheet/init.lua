...
function ENT:Use( activator, caller )	
	if IsValid(caller) and caller:IsPlayer()  then
		...
		local Count=0
		for k, v in pairs( ents.FindByClass( "psychedelics_blotter_5sheet" ) ) do
			if v:CPPIGetOwner()==activator then
				Count=Count+1
			end
		end
		if Count>=GetConVar("psychedelics_limitspawn_5sheet"):GetInt() then
		...
		for i=1,5 do
			...
			sheet5:Spawn()
			sheet5:Activate()
			sheet5:SetNWString("psychedelics_blotter_data","psychedelics_blotter_".."-"..subMaterial.."-"..tostring(i))
			...
			duplicator.StoreEntityModifier( sheet5, "psychedelics_blotter_data", {data="psychedelics_blotter_".."-"..subMaterial.."-"..tostring(i), matpos=matpos,
