...
function ENT:Initialize()
		...
		timer.Simple(0.1,function()
			Ug=self:GetNWInt( "psychedelics_quantity", 100 )
			local sheet5=self
			local matpos=sheet5:GetNWInt("matpos",1)
			local data=sheet5:GetNWString("psychedelics_blotter_data","psychedelics_blotter_-psychedelics/blotters/bycicle_day-1")
...
function ENT:Use( activator, caller )
	if IsValid(caller) and caller:IsPlayer()  then
		...
		local Count=0
		for k, v in pairs( ents.FindByClass( "psychedelics_blotter_1sheet" ) ) do
			if v:CPPIGetOwner()==activator then
				Count=Count+1
			end
		end
		if Count>=GetConVar("psychedelics_limitspawn_1sheet"):GetInt() then
		...
		local data=self:GetNWString("psychedelics_blotter_data","psychedelics_blotter_-psychedelics/blotters/bycicle_day-1")
		...
		for i=1,5 do
			...
			sheet1:Spawn()
			sheet1:Activate()
			sheet1:SetNWString("psychedelics_blotter_data","psychedelics_blotter_".."-"..subMaterial.."-"..tostring(x).."-"..tostring(i))
			...
			duplicator.StoreEntityModifier( sheet1, "psychedelics_blotter_data", {data="psychedelics_blotter_".."-"..subMaterial.."-"..tostring(x).."-"..tostring(i), matpos=matpos,
