...
function ENT:Initialize()
		self:SetModel("models/psychedelics_mod/lsd/blotter/5sheet.mdl")
		...
		Ug=self:GetNWInt( "psychedelics_mod_quantity", 100 )
		local sheet5=self
		local matpos=sheet5:GetNWInt("matpos",1)
		local data=sheet5:GetNWString("psychedelics_mod_blotter_data","psychedelics_mod_blotter_-psychedelics_mod/blotters/bycicle_day-1")
...
function ENT:Use( activator, caller )
	if IsValid(caller) and caller:IsPlayer()  then
		...
		local Count=0
		for k, v in pairs( ents.FindByClass( "psychedelics_mod_blotter_1sheet" ) ) do
			if v:CPPIGetOwner()==activator then
				Count=Count+1
			end
		end
		if Count>=GetConVar("psychedelics_mod_limitspawn_1sheet"):GetInt() then
		...
		self:Remove()
		for i=1,5 do
			local sheet1 = ents.Create("psychedelics_mod_blotter_1sheet")
			sheet1:SetPos( Pos-sheet5:GetForward()*(2*i)+(sheet5:GetForward()*6))
			sheet1:CPPISetOwner(activator)
			sheet1:SetAngles( angles5)
			sheet1:Spawn()
			sheet1:Activate()
			sheet1:SetNWString("psychedelics_mod_blotter_data","psychedelics_mod_blotter_".."-"..subMaterial.."-"..tostring(x).."-"..tostring(i))
			sheet1:SetNWInt("matpos",matpos)
			sheet1:SetNWString("psychedelics_mod_type",thetype)
			sheet1:SetNWInt("psychedelics_mod_quantity",thequantity)
			duplicator.StoreEntityModifier( sheet1, "psychedelics_mod_blotter_data", {data="psychedelics_mod_blotter_".."-"..subMaterial.."-"..tostring(x).."-"..tostring(i), matpos=matpos,
			...
			timer.Simple(0.1,function()
				...
				net.WriteString(pos1)
				net.WriteString(pos2)
				net.WriteString("psychedelics_mod_1sheet_".."-"..tmaterial[#tmaterial].."-"..tostring(matpos).."-"..tostring(x).."-"..tostring(i))
